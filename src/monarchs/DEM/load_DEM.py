...
def interpolate_DEM(heights, num_points):
    ...
    interp = RegularGridInterpolator((x, y), heights, bounds_error=False,
        fill_value=None)
...
def export_DEM(tiffname, num_points=50, top_right=False, top_left=False,
    bottom_right=False, bottom_left=False, diagnostic_plots=False,
    all_outputs=False, return_lats=True, input_crs=3031):
    """
    ...
    transformer = Transformer.from_crs(input_crs, CRS.from_epsg(4326),
        always_xy=True)
    ...
    if not any(val in [False, np.nan] for val in np.ravel([top_right,
        ...
        subset_raster = projected_raster.rio.clip_box(minx=lon_min, miny=
            lat_min, maxx=lon_max, maxy=lat_max, crs='EPSG:4326')
        ...
        if len(lat_subset) != 0 and len(lon_subset) != 0:
            ...
            if diagnostic_plots:
                bounding_box_diagnostic_plots(input_raster, subset_raster,
                    lon_array, lat_array, lon_subset, lat_subset)
    ...
    if diagnostic_plots:
        generate_diagnostic_plots(lon_array, lat_array, heights, newlons,
            newlats, new_heights, new_heights_interpolated)
    if all_outputs:
        return (heights, lat_array, lon_array, new_heights,
            new_heights_interpolated, newlons, newlats, dx, dy)
    elif return_lats:
...
def get_xy_distance(latitudes, longitudes):
    ...
    _, _, dx_metres = geod.inv(lon_grid[:, :-1], lat_grid[:, :-1], lon_grid
        [:, 1:], lat_grid[:, 1:])
    _, _, dy_metres = geod.inv(lon_grid[:-1, :], lat_grid[:-1, :], lon_grid
        [1:, :], lat_grid[1:, :])
...
def bounding_box_diagnostic_plots(input_raster, subset_raster, lon_array,
    lat_array, lon_subset, lat_subset):
    """
    ...
    levels = np.arange(np.nanmin(subset_raster.values[0]), np.nanmax(
        subset_raster.values[0]), 1)
    cont = ax1.contourf(lon_array, lat_array, input_raster.values[0], cmap=
        cmap, transform=ccrs.PlateCarree(), vmax=np.nanmax(subset_raster.
        values[0]), vmin=np.nanmin(subset_raster.values[0]), levels=levels)
    ax1.coastlines()
    ax1.gridlines(draw_labels=True)
    ax1.title.set_text('DEM height profile with subset')
    cont = ax1.contourf(lon_subset, lat_subset, subset_raster.values[0],
        cmap='magma', transform=ccrs.PlateCarree(), levels=levels)
    plt.figure()
    plt.imshow(subset_raster.values[0])
    plt.figure()
...
def generate_diagnostic_plots(lons, lats, heights, newlons, newlats,
    newheights, new_heights_interpolated):
    """
    ...
    cont = ax1.contourf(lons, lats, heights, cmap=cmap, transform=ccrs.
        PlateCarree())
    ax1.coastlines()
    ax1.gridlines(draw_labels=True)
    ax1.title.set_text('Initial DEM height profile')
    ax2 = fig.add_subplot(212, projection=projection)
    cont2 = ax2.contourf(newlons, newlats, newheights, cmap=cmap, transform
        =projection, levels=bounds, vmin=0, vmax=500)
    ax2.coastlines()
    ...
    ax3.title.set_text(
        f'After interpolating to our grid size ({len(new_heights_interpolated)}x{len(new_heights_interpolated[0])})'
        )
    cb = fig.colorbar(cs)
    plt.show()
...
if __name__ == '__main__':
    ...
    (heights, lats, lons, newheights, new_heights_interpolated, newlons,
        newlats, dx, dy) = (export_DEM(tiffname, num_points=50,
        diagnostic_plots=False, top_right=[(-66.52, -62.814)], bottom_left=
        [(-66.289, -64.68)], top_left=[(-66.04, -63.42)], bottom_right=[(-
        66.778, -64.099)], all_outputs=True))
    import matplotlib.pyplot as plt
    generate_diagnostic_plots(lons, lats, heights, newlons, newlats,
        newheights, new_heights_interpolated)
