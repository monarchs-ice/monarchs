...
def get_xy_distance(latitudes, longitudes):
    ...
    _, _, dx_metres = geod.inv(
        lon_grid[:, :-1], lat_grid[:, :-1], lon_grid[:, 1:], lat_grid[:, 1:]
    )
    _, _, dy_metres = geod.inv(
        lon_grid[:-1, :], lat_grid[:-1, :], lon_grid[1:, :], lat_grid[1:, :]
    )
...
def bounding_box_diagnostic_plots(
    ...
    levels = np.arange(
        np.nanmin(subset_raster.values[0]), np.nanmax(subset_raster.values[0]), 1
    )
    cont = ax1.contourf(
        lon_array,
        lat_array,
...
def generate_diagnostic_plots(
    ...
    cont = ax1.contourf(lons, lats, heights, cmap=cmap, transform=ccrs.PlateCarree())
    ax1.coastlines()
    ax1.gridlines(draw_labels=True)
...
if __name__ == "__main__":
    ...
    (
        ...
        num_points=50,
        diagnostic_plots=False,
        top_right=[(-66.52, -62.814)],
        bottom_left=[(-66.289, -64.68)],
        top_left=[(-66.04, -63.42)],
        bottom_right=[(-66.778, -64.099)],
        all_outputs=True,
    )
