...
def snowfall(cell, snow_depth, snow_rho, snow_T):
    ...
    if snow_depth != 0:
        if cell['lake']:
            ...
            orig_liquid = np.sum(cell['Lfrac'] * cell['rho_water'] * (cell[
                'firn_depth'] / cell['vert_grid']))
            orig_solid = np.sum(cell['Sfrac'] * cell['rho_ice'] * (cell[
                'firn_depth'] / cell['vert_grid']))
            ...
            weight1 = dz_old * (cell['Sfrac'][0] * cell['rho_ice'] + cell[
                'Lfrac'][0] * cell['rho_water'])
            weight2 = snow_depth * snow_rho
            cell['firn_temperature'][0] = (weight1 * cell[
                'firn_temperature'][0] + weight2 * snow_T) / (weight1 + weight2
                )
            weight1 = snow_depth
            weight2 = dz_old
            cell['Sfrac'][0] = (weight1 * (snow_rho / cell['rho_ice']) + 
                weight2 * cell['Sfrac'][0]) / (weight1 + weight2)
            ...
            for i in range(1, len(cell['Sfrac'])):
                weight_1 = cell['firn_depth'] - i * dz_new - (old_firn_depth -
                    i * dz_old)
                weight_2 = old_firn_depth - i * dz_old - (cell['firn_depth'
                    ] - (i + 1) * dz_new)
                lfrac_hold[i] = (cell['Lfrac'][i - 1] * weight_1 + cell[
                    'Lfrac'][i] * weight_2) / (weight_1 + weight_2)
                sfrac_hold[i] = (cell['Sfrac'][i - 1] * weight_1 + cell[
                    'Sfrac'][i] * weight_2) / (weight_1 + weight_2)
                T_hold[i] = (cell['firn_temperature'][i - 1] * weight_1 + 
                    cell['firn_temperature'][i] * weight_2) / (weight_1 +
                    weight_2)
            ...
            assert abs(calc_mass_sum(cell) - (original_mass + snow_depth *
                snow_rho)) < 1.5 * 10 ** -7
