    ...
    if not cell['valid_cell']:
        ...
        if cell['ice_lens']:
            if not np.any(cell['saturation'][:cell['ice_lens_depth'] + 1] > 0):
                ...
                top_saturation_depth = np.argmax(cell['saturation'][:cell[
                    'ice_lens_depth'] + 1] > 0)
...
def get_neighbour_water_levels(cell, grid, col, row, max_grid_col, max_grid_row
    ...
    neighbours = {}
    if row > 0:
        neighbours['N'] = cell['water_level'] - grid['water_level'][row - 1][
            col]
        if col < max_grid_col - 1:
            neighbours['NE'] = cell['water_level'] - grid['water_level'][
                row - 1][col + 1]
        if col > 0:
            neighbours['NW'] = cell['water_level'] - grid['water_level'][
                row - 1][col - 1]
    else:
        neighbours['N'] = -999
        neighbours['NE'] = -999
    ...
    if row < max_grid_row - 1:
        neighbours['S'] = cell['water_level'] - grid['water_level'][row + 1][
            col]
        if col < max_grid_col - 1:
            neighbours['SE'] = cell['water_level'] - grid['water_level'][
                row + 1][col + 1]
        if col > 0:
            neighbours['SW'] = cell['water_level'] - grid['water_level'][
                row + 1][col - 1]
    else:
        neighbours['S'] = -999
        neighbours['SE'] = -999
        neighbours['SW'] = -999
    if col > 0:
        neighbours['W'] = cell['water_level'] - grid['water_level'][row][
            col - 1]
    else:
        neighbours['W'] = -999
        neighbours['SW'] = -999
        neighbours['NW'] = -999
    if col < max_grid_col - 1:
        neighbours['E'] = cell['water_level'] - grid['water_level'][row][
            col + 1]
    else:
        neighbours['E'] = -999
        neighbours['SE'] = -999
...
def find_biggest_neighbour(cell, grid, col, row, max_grid_col, max_grid_row,
    catchment_outflow, flow_into_land=False):
    """
    ...
    """
    neighbours = get_neighbour_water_levels(cell, grid, col, row,
        max_grid_col, max_grid_row)
    ...
    max_list = [k for k, v in neighbours.items() if v ==
...
def calc_available_water(cell, water_frac, split, neighbour_cell=False,
    outflow=False):
    """
    ...
    if cell['lake']:
        ...
        water_to_move = water_frac * ((cell['water_level'] - neighbour_cell
            ['water_level']) / (split + 1))
        ...
        if outflow:
            lowest_water_level = cell['vertical_profile'][::-1][cell[
                'ice_lens_depth']]
            ...
            return water_to_move
        top_saturation_level = np.argmax(cell['saturation'][:cell[
            'ice_lens_depth'] + 1] > 0)
        if cell['vertical_profile'][::-1][cell['ice_lens_depth']
            ] > neighbour_cell['water_level']:
            lowest_water_level = cell['vertical_profile'][::-1][cell[
                'ice_lens_depth']]
        else:
            lowest_water_level = cell['vertical_profile'][::-1][find_nearest
                (cell['vertical_profile'][::-1], (cell['water_level'] +
                neighbour_cell['water_level']) / (split + 1))]
        vp = cell['vertical_profile'][::-1]
        move_from_index = find_nearest(vp, lowest_water_level)
        water_to_move = water_frac[move_from_index] * (cell['water_level'] -
            neighbour_cell['water_level']) / (split + 1)
        if np.sum(cell['water'][:move_from_index + 1]) / (split + 1
            ) < water_to_move:
            water_to_move = np.sum(cell['water'][:move_from_index + 1]) / (
                split + 1)
        return (water_to_move, lowest_water_level, move_from_index,
...
def move_to_neighbours(grid, temp_grid, biggest_neighbours, col, row, split,
    catchment_outflow, biggest_height_difference, timestep, flow_into_land=
    False):
    """
    ...
    """
    all_neighbours = {'NW': [-1, -1], 'N': [-1, 0], 'NE': [-1, 1], 'E': [0,
        1], 'SE': [1, 1], 'S': [1, 0], 'SW': [1, -1], 'W': [0, -1]}
    for idx, neighbour in enumerate(all_neighbours.keys()):
        if neighbour in biggest_neighbours:
            ...
            water_frac = water_fraction(cell, biggest_height_difference,
                timestep, neighbour)
            try:
                if (row + n_s_index == -1 or col + w_e_index == -1 or row +
                    n_s_index >= len(grid) or col + w_e_index >= len(grid[0])):
                    raise IndexError
                neighbour_cell = grid[row + n_s_index][col + w_e_index]
                temporary_neighbour = temp_grid[row + n_s_index][col +
                    w_e_index]
                if not neighbour_cell['valid_cell'] and not flow_into_land:
                    continue
                elif not neighbour_cell['valid_cell'] and flow_into_land:
                    water_out = calc_catchment_outflow(cell, temporary_cell,
                        water_frac, split)
                    if water_out > 0:
                ...
                if cell['lid'] or neighbour_cell['lid']:
                    continue
                if cell['lake']:
                    water_to_move = calc_available_water(cell, water_frac,
                        split, neighbour_cell=neighbour_cell)
                elif cell['ice_lens'] and not cell['lake']:
                    (water_to_move, lowest_water_level, move_from_index,
                        top_saturation_level) = (calc_available_water(cell,
                        water_frac, split, neighbour_cell=neighbour_cell))
            except IndexError:
            ...
            if cell['lake']:
                ...
                elif cell['firn_depth'] + cell['lake_depth'] > neighbour_cell[
                    'firn_depth']:
                    temporary_neighbour.water[0] += water_to_move
                else:
                    move_to_index = find_nearest(neighbour_cell[
                        'vertical_profile'][::-1], cell['water_level'])
                ...
                if not neighbour_cell['lake']:
                    move_to_index = find_nearest(neighbour_cell[
                        'vertical_profile'][::-1], lowest_water_level)
                else:
                    move_to_index = 0
                for idx in range(top_saturation_level, move_from_index + 1):
                    if cell['water'][idx] / (split + 1) > water_to_move:
                        ...
                        if not neighbour_cell['lake']:
                            temporary_neighbour.water[move_to_index
                                ] += water_to_move
                        ...
                        temporary_cell.water[idx] -= cell['water'][idx] / (
                            split + 1)
                        water_to_move -= cell['water'][idx] / split
                        if not neighbour_cell['lake']:
                            temporary_neighbour.water[move_to_index] += cell[
                                'water'][idx] / (split + 1)
                            temporary_neighbour.meltflag[move_to_index] = 1
                        else:
...
def move_water(grid, max_grid_col, max_grid_row, timestep,
    catchment_outflow=True, lateral_movement_percolation_toggle=True,
    ...
    for row in range(max_grid_row):
        ...
        for col in range(max_grid_col):
            _l.append(TemporaryCell(np.copy(grid['lake_depth'][row][col]),
                np.copy(grid['water'][row][col]), np.copy(grid['saturation'
                ][row][col]), np.copy(grid['meltflag'][row][col]), np.copy(
                grid['lake'][row][col])))
        temp_grid.append(_l)
    for row in range(max_grid_row):
        for col in range(max_grid_col):
            cell = grid[row][col]
            if cell['valid_cell'] and (cell['ice_lens'] and (cell['water'] >
                0).any() or cell['lake'] and cell['lake_depth'] > 0):
                biggest_height_difference, max_list = find_biggest_neighbour(
                    cell, grid, col, row, max_grid_col, max_grid_row,
                    catchment_outflow, flow_into_land=flow_into_land)
                split = len(max_list)
                if biggest_height_difference > 0:
                    catchment_out_water += move_to_neighbours(grid,
                        temp_grid, max_list, col, row, split,
                        catchment_outflow, biggest_height_difference,
                        timestep, flow_into_land=flow_into_land)
            if (cell['water'] < 0).any():
    ...
    for row in range(0, max_grid_row):
        for col in range(0, max_grid_col):
            ...
            if cell['valid_cell']:
                cell['Lfrac'] = cell['water'] / (cell['firn_depth'] / cell[
                    'vert_grid'])
