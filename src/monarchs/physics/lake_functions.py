...
def sfc_energy_lake_formation(T_air, Q, k, cell):
    ...
    args = np.array([cell['firn_depth'], cell['vert_grid'], Q, k[0], cell[
        'firn_temperature'][1]])
...
def turbulent_mixing(cell, SW_in, dt):
    ...
    J = 0.1 * (9.8 * 5 * 10 ** -5 * (1.19 * 10 ** -7) ** 2 / 10 ** -6) ** (
        1 / 3)
    albedo = sfc_albedo(cell['melt'], cell['exposed_water'], cell['lid'],
        cell['lake'], cell['lake_depth'])
    Int = (1 - albedo) * SW_in * np.exp(-tau * cell['lake_depth']) - (1 -
        albedo) * SW_in * np.exp(-tau * 0)
    dt_scaling = 2
    for i in range(int(dt / dt_scaling)):
        T_core = cell['lake_temperature'][int(cell['vert_grid_lake'] / 2)]
        Fu = np.sign(T_core - cell['lake_temperature'][0]
            ) * 1000 * 4181 * J * abs(T_core - cell['lake_temperature'][0]
            ) ** (4 / 3)
        Fl = np.sign(T_core - 273.15) * 1000 * 4181 * J * abs(T_core - 273.15
            ) ** (4 / 3)
...
def lake_formation(cell, dt, LW_in, SW_in, T_air, p_air, T_dp, wind,
    ...
    for i in np.arange(0, cell['vert_grid']):
        if cell['firn_temperature'][i] > 273.15:
            ...
            cp_ice[i] = 1000 * (7.16 * 10 ** -3 * cell['firn_temperature'][
                i] + 0.138)
            k_ice[i] = 1000 * (2.24 * 10 ** -3 + 5.975 * 10 ** -6 * (273.15 -
                cell['firn_temperature'][i]) ** 1.156)
        air[i] = 1 - cell['Sfrac'][i] - cell['Lfrac'][i]
    k = cell['Sfrac'] * k_ice + air * cell['k_air'] + cell['Lfrac'] * cell[
        'k_water']
    ...
    if old_T_sfc >= 273.15 and Q > 0:
        kdTdz = (cell['firn_temperature'][0] - cell['firn_temperature'][1]
            ) * abs(k[0]) / (cell['firn_depth'] / cell['vert_grid'])
        dHdt = (Q - kdTdz) / (cell['Sfrac'][0] * cell['L_ice'] * cell[
            'rho_ice']) * dt
        if dHdt < 0:
            raise ValueError(
    ...
    root, fvec, success, info = solver.firn_heateqn_solver(x, args,
        fixed_sfc=True, solver_method=toggle_dict['solver'])
    if success:
...
def lake_development(cell, dt, LW_in, SW_in, T_air, p_air, T_dp, wind,
    ...
    if not cell['v_lid'] and not cell['lid']:
        x = cell['lake_temperature']
        Q = sfc_flux(cell['melt'], cell['exposed_water'], cell['lid'], cell
            ['lake'], cell['lake_depth'], LW_in, SW_in, T_air, p_air, T_dp,
            wind, x[0])
    ...
    if cell['lake_temperature'][-2] > 273.15:
        if cell['firn_depth'] > 0:
            kdTdz = (273.15 - cell['lake_temperature'][-2]) * abs(k[0]) / (
                2 * (cell['firn_depth'] / cell['vert_grid']))
            boundary_change = -kdTdz / (cell['Sfrac'][0] * cell['L_ice'] *
                cell['rho_ice']) * dt
    ...
    if cell['lake_depth'] > 0:
        ...
        print('Lake depth = ', cell['lake_depth'], 'column = ', cell[
            'column'], 'row = ', cell['row'])
        return
    cell['lake_temperature'] = np.interp(new_depth_grid, old_depth_grid,
