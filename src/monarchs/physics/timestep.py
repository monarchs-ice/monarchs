...
def timestep_loop(cell, dt, met_data, t_steps_per_day, toggle_dict):
    ...
    for t_step in range(t_steps_per_day):
        ...
        if snowfall_toggle:
            ...
            snow_accumulation.snowfall(cell, met_data.snowfall[t_step],
                met_data.snow_dens[t_step], 260)
        ...
        if not cell['exposed_water'] and not cell['saturation'][0]:
            if firn_column_toggle:
                firn_functions.firn_column(cell, dt, dz, LW_in, SW_in,
            ...
            if firn_heat_toggle:
                sol, fvec, success, info = solver.firn_heateqn_solver(x,
            ...
            if cell['lake'] and not cell['lid'] and not cell['v_lid'] and cell[
                'lake_depth'] < 0.1 and cell['v_lid_depth'] <= 0:
                cell['lake'] = False
            if cell['lake_depth'] == 0:
            ...
            if cell['lake'] is False:
                ...
                if lid_development_toggle:
                    cell['v_lid'] = False
                    if lake_development_toggle:
                        lake_functions.lake_development(cell, dt, LW_in,
                            SW_in, T_air, p_air, T_dp, wind, toggle_dict)
                    lid_functions.lid_development(cell, dt, LW_in, SW_in,
                        T_air, p_air, T_dp, wind)
