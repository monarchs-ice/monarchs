...
def initialise_iceshelf(
    ...
    iceshelf["vertical_profile"] = np.moveaxis(np.linspace(0, firn_depth, vert_grid), 0, -1)
    ...
    if np.isnan(lake_temperature).any():
        iceshelf["lake_temperature"] = 273.15 * np.ones((num_rows, num_cols, vert_grid_lake))
    else:
        iceshelf["lake_temperature"][:] = lake_temperature

    if np.isnan(lid_temperature).any():
        iceshelf["lid_temperature"] = 273.15 * np.ones((num_rows, num_cols, vert_grid_lid))
    else:
    ...
    # always starts off at 0 and is calculated in lateral functions
...
def get_spec(num_rows, num_cols, vert_grid_size, vert_grid_lid, vert_grid_lake):
    ...
    dtype = np.dtype([
        ...
        ("vert_grid", np.int32),
        ("vertical_profile", np.float64, vert_grid_size),  # Example fixed size for vert_grid
        ("vert_grid_lake", np.int32),
        ("vert_grid_lid", np.int32),
        ("rho", np.float64, vert_grid_size),  # Example fixed size for vert_grid
        ("rho_lid", np.float64, vert_grid_lid),  # Example fixed size for vert_grid_lid
        ...
        ("t_step", np.int32),
        ("day", np.int32),
        ("log", "U256"),  # String field with max length 256
