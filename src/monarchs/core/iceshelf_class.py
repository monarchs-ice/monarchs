...
def initialise_iceshelf(num_rows, num_cols, vert_grid, vert_grid_lake,
    vert_grid_lid, dtype, x, y, firn_depth, rho, firn_temperature, Sfrac=np
    .array([np.nan]), Lfrac=np.array([np.nan]), meltflag=np.array([np.nan]),
    saturation=np.array([np.nan]), lake_depth=0, lake_temperature=np.array(
    [np.nan]), lid_depth=0, lid_temperature=np.array([np.nan]), melt=False,
    exposed_water=False, lake=False, v_lid=False, lid=False, water_level=0,
    water=np.array([np.nan]), ice_lens=False, ice_lens_depth=999,
    has_had_lid=False, lid_sfc_melt=0, lid_melt_count=0, melt_hours=0,
    exposed_water_refreeze_counter=0, virtual_lid_temperature=273.15,
    total_melt=0.0, valid_cells=True, lat=0, lon=0, numba=False, size_dx=
    1000, size_dy=1000):
    """
    ...
    iceshelf['vertical_profile'] = np.moveaxis(np.linspace(0, firn_depth,
        vert_grid), 0, -1)
    ...
    if np.isnan(Sfrac).all():
        iceshelf['Sfrac'] = np.ones((num_rows, num_cols, vert_grid)
            ) * rho / 917
    else:
    ...
    if np.isnan(lake_temperature).any():
        iceshelf['lake_temperature'] = 273.15 * np.ones((num_rows, num_cols,
            vert_grid_lake))
    else:
        iceshelf['lake_temperature'][:] = lake_temperature
    if np.isnan(lid_temperature).any():
        iceshelf['lid_temperature'] = 273.15 * np.ones((num_rows, num_cols,
            vert_grid_lid))
    else:
...
def get_spec(num_rows, num_cols, vert_grid_size, vert_grid_lid, vert_grid_lake
    ...
    """
    dtype = np.dtype([('column', np.int32), ('row', np.int32), (
        'firn_depth', np.float64), ('vert_grid', np.int32), (
        'vertical_profile', np.float64, vert_grid_size), ('vert_grid_lake',
        np.int32), ('vert_grid_lid', np.int32), ('rho', np.float64,
        vert_grid_size), ('rho_lid', np.float64, vert_grid_lid), (
        'firn_temperature', np.float64, vert_grid_size), ('Sfrac', np.
        float64, vert_grid_size), ('Lfrac', np.float64, vert_grid_size), (
        'meltflag', np.float64, vert_grid_size), ('saturation', np.float64,
        vert_grid_size), ('lake_temperature', np.float64, vert_grid_lake),
        ('lid_temperature', np.float64, vert_grid_lid), ('water_level', np.
        float64), ('water', np.float64, vert_grid_size), ('melt', np.bool_),
        ('exposed_water', np.bool_), ('lake', np.bool_), ('lake_depth', np.
        float64), ('v_lid', np.bool_), ('virtual_lid_temperature', np.
        float64), ('lid', np.bool_), ('lid_depth', np.float64), ('ice_lens',
        np.bool_), ('ice_lens_depth', np.int32), ('rho_ice', np.float64), (
        'rho_water', np.float64), ('L_ice', np.float64), ('pore_closure',
        np.float64), ('k_air', np.float64), ('cp_air', np.float64), (
        'k_water', np.float64), ('cp_water', np.float64), ('v_lid_depth',
        np.float64), ('has_had_lid', np.bool_), ('melt_hours', np.int32), (
        'exposed_water_refreeze_counter', np.int32), ('lid_sfc_melt', np.
        float64), ('lid_melt_count', np.int32), ('total_melt', np.float64),
        ('t_step', np.int32), ('day', np.int32), ('log', 'U256'), (
        'snow_added', np.float64), ('reset_combine', np.bool_), (
        'valid_cell', np.bool_), ('lat', np.float64), ('lon', np.float64),
        ('size_dx', np.float64), ('size_dy', np.float64), ('numba', np.bool_)])
    return dtype
