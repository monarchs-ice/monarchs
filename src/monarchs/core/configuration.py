...
def parse_args():
    ...
    parser = argparse.ArgumentParser(prog='MONARCHS', description=
        'A model of ice shelf development, written by Sammie Buzzard, Jon Elsey and Alex Robel.'
        )
    parser.add_argument('--input_path', '-i', help=
        'Absolute or relative path to an input file, in the formatof <model_setup.py>'
        , default='model_setup.py', required=False)
...
def handle_incompatible_flags(model_setup):
    ...
    """
    print('\n')
    if hasattr(model_setup, 'lat_bounds') and not hasattr(model_setup,
        'DEM_path'):
    ...
    for attr in dump_attrs:
        if hasattr(model_setup, attr) and not hasattr(model_setup,
            'dump_filepath'):
    ...
    if hasattr(model_setup, 'dump_format'):
        ...
        if model_setup.dump_format == 'pickle' and hasattr(model_setup,
            'use_numba') and model_setup.use_numba:
            raise ValueError(
...
def create_defaults_for_missing_flags(model_setup):
    ...
    None
    """
    optional_args_to_true = ['lake_development_toggle',
        'lid_development_toggle', 'lateral_movement_toggle',
        'lateral_movement_percolation_toggle', 'percolation_toggle',
        'flow_into_land', 'perc_time_toggle', 'snowfall_toggle',
        'firn_heat_toggle', 'firn_column_toggle', 'single_column_toggle']
    optional_args_to_false = ['densification_toggle',
        'simulated_water_toggle', 'ignore_errors', 'heateqn_res_toggle',
        'dump_data', 'verbose_logging', 'spinup', 'reload_from_dump',
        'met_dem_diagnostic_plots', 'bbox_top_right', 'bbox_bottom_left',
        'bbox_top_left', 'bbox_bottom_right',
        'dump_data_pre_lateral_movement', 'use_numba', 'use_mpi',
        'dem_diagnostic_plots', 'parallel', 'use_numba', 'catchment_outflow']
    ...
    vardict['vars_to_save'] = ('firn_temperature', 'Sfrac', 'Lfrac',
        'firn_depth', 'lake_depth', 'lid_depth', 'lake', 'lid', 'v_lid',
        'ice_lens_depth')
    ...
    elif hasattr(model_setup, 'met_data') and isinstance(model_setup.
        met_data, dict):
...
def jit_modules():
    ...
    module_list = [surface_fluxes, utils, firn_functions, lake_functions,
        lid_functions, percolation_functions, snow_accumulation, timestep]
