...
def setup_output(fname, grid, vars_to_save=('firn_temperature', 'Sfrac',
    'Lfrac', 'firn_depth', 'lake_depth', 'lid_depth'), vert_grid_size=False):
    with Dataset(fname, clobber=True, mode='w') as data:
        ...
        for key in vars_loop:
            ...
            if key in dims:
                ...
                if ('lake' in key and 'lake_depth' not in key and key not in
                    dims):
                    var_write = data.createVariable(key, dtype, ('time',
                        'x', 'y', 'vert_grid_lake'))
                elif 'lid' in key and 'lid_depth' not in key and key not in dims:
                    var_write = data.createVariable(key, dtype, ('time',
                        'x', 'y', 'vert_grid_lid'))
                else:
                    new_var = np.zeros((len(grid['firn_depth']), len(grid[
                        'firn_depth'][0]), vert_grid_size))
                    if vert_grid_size != grid['vert_grid'][0][0]:
                        for i in range(len(grid['firn_depth'])):
                            for j in range(len(grid['firn_depth'][i])):
                                new_var[i][j] = np.interp(np.linspace(0,
                                    grid['firn_depth'][i][j],
                                    vert_grid_size), np.linspace(0, grid[
                                    'firn_depth'][i][j], grid['vert_grid'][
                                    i][j]), var[i][j])
                    var = new_var
                    var_write = data.createVariable(key, dtype, ('time',
                        'x', 'y', 'vert_grid'))
...
def update_model_output(fname, grid, iteration, vars_to_save=(
    'firn_temperature', 'Sfrac', 'Lfrac', 'firn_depth', 'lake_depth',
    'lid_depth'), hourly=False, t_step=0, vert_grid_size=False):
    if not hourly:
        index = iteration
    else:
        ...
        for key in vars_to_save:
            ...
            if 'vert_grid' in var_write.dimensions:
                if vert_grid_size != grid['vert_grid'][0][0
                    ...
                    for i in range(len(grid['firn_depth'])):
                        for j in range(len(grid['firn_depth'][i])):
                            new_var[i][j] = np.interp(np.linspace(0, grid[
                                'firn_depth'][i][j], vert_grid_size), np.
                                linspace(0, grid['firn_depth'][i][j], grid[
                                'vert_grid'][i][j]), var[i][j])
                    var = new_var
            var_write[index] = var
