...
def setup_era5(model_setup, lat_array=False, lon_array=False):
    ...
    for year in range(model_years):
        ...
        print(f'monarchs.core.initial_conditions.setup_era5: Writing data for year {year + 1} into netCDF...')
        write_to_netcdf(ERA5_grid_path, ERA5_grid, model_setup, start_index=start_index)
    print(
    ...
    with Dataset(model_setup.met_output_filepath, "w", comm=comm) as f:
        ...
        if "snow_dens" not in met_data:
            met_data["snow_dens"] = np.broadcast_to(
                300 * np.ones(model_setup.num_days * model_setup.t_steps_per_day),
                (
