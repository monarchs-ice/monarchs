...
import numpy as np
from monarchs.met_data.import_ERA5 import ERA5_to_variables, interpolate_grid, grid_subset, get_met_bounds_from_DEM
...
def setup_era5(model_setup, lat_array=False, lon_array=False):
    ...
    if model_setup.met_timestep == 'hourly':
        ...
        raise ValueError(
            f'monarchs.core.initial_conditions.setup_era5: met_timestep should be an integer, "hourly", "three_hourly" or "daily". See documentation for model_setup.met_timestep for details.'
            )
    ERA5_vars = ERA5_to_variables(model_setup.met_input_filepath)
    bounds = ['latmax', 'latmin', 'longmax', 'longmin']
    if all(hasattr(model_setup, attr) for attr in bounds) and all(~np.isnan
        (getattr(model_setup, attr)) for attr in bounds):
        ERA5_vars = grid_subset(ERA5_vars, model_setup.latmax, model_setup.
            latmin, model_setup.longmax, model_setup.longmin)
    ERA5_grid = interpolate_grid(ERA5_vars, model_setup.row_amount,
        model_setup.col_amount)
    if hasattr(model_setup, 'lat_bounds') and model_setup.lat_bounds.lower(
        ) == 'dem':
        ERA5_grid = get_met_bounds_from_DEM(model_setup, ERA5_grid,
            lat_array, lon_array, diagnostic_plots=model_setup.
    ...
    if index > 1:
        selected_keys = [key for key in ERA5_grid.keys() if key not in [
            'lat', 'long']]
    ...
    with Dataset(ERA5_grid_path, 'w', comm=comm) as f:
        ...
        for key, value in ERA5_grid.items():
            if key in ['long', 'lat', 'time']:
                if key == 'long':
                    var = f.createVariable('cell_longitude', np.dtype(
                        'float64').char, ('column', 'row'))
                    var.long_name = 'Longitude of grid cell'
                    var[:] = value
                if key == 'lat':
                    var = f.createVariable('cell_latitude', np.dtype(
                        'float64').char, ('column', 'row'))
                    var.long_name = 'Latitude of grid cell'
                    var[:] = value
                else:
                    continue
            var = f.createVariable(key, np.dtype('float64').char, ('time',
                'column', 'row'))
            var.long_name = key
            var[:] = value
        print(
            f'monarchs.core.initial_conditions.setup_era5: Saved meteorological data used for the model run into {ERA5_grid_path}'
            )
...
def prescribed_met_data(model_setup):
    ...
    with Dataset(model_setup.met_output_filepath, 'w', comm=comm) as f:
        f.createGroup('variables')
        f.createDimension('time', model_setup.num_days * model_setup.
            t_steps_per_day)
        f.createDimension('column', model_setup.col_amount)
        f.createDimension('row', model_setup.row_amount)
        if 'lat' not in met_data:
            if 'latitude' in met_data:
                ...
                met_data['lat'] = np.zeros((model_setup.row_amount,
                    model_setup.col_amount))
        if 'long' not in met_data:
            if 'longitude' in met_data:
                met_data['long'] = met_data['longitude']
            else:
                met_data['long'] = np.zeros((model_setup.row_amount,
                    model_setup.col_amount))
        if 'snow_dens' not in met_data:
            met_data['snow_dens'] = np.broadcast_to(300 * np.ones(
                model_setup.num_days * model_setup.t_steps_per_day), (
                model_setup.row_amount, model_setup.col_amount, len(
                met_data['snow_dens'])))
        for key, value in met_data.items():
            ...
            var[:] = value
        print(
            f'monarchs.core.initial_conditions.setup_prescribed_data: Saved meteorological data used for the model run into {model_setup.met_output_filepath}'
            )
