...
def ERA5_to_variables(ERA5_input):
    ...
    var_dict['wind'] = np.sqrt(ERA5_data.variables['u10'][:] ** 2 + 
        ERA5_data.variables['v10'][:] ** 2)
    ...
    try:
        ...
        try:
            var_dict['pressure'] = ERA5_data.variables['msl'][:] / 100
        except:
            raise KeyError(
    ...
    try:
        ...
        try:
            var_dict['SW_surf'] = ERA5_data.variables['ssrdc'][:] / 3600
            print(
    ...
    try:
        ...
        try:
            ...
            var_dict['LW_surf'] = ERA5_data.variables['strdc'][:] / 3600
        except:
            raise KeyError(
    ...
    try:
        ...
        var_dict['snow_albedo'] = 0.85 * np.ones(np.shape(ERA5_data.
            variables['t2m']))
    try:
        var_dict['snow_dens'] = ERA5_data.variables['rsn'][:]
    except KeyError:
        var_dict['snow_dens'] = 300 * np.ones(np.shape(ERA5_data.variables[
            't2m']))
    ERA5_data.close()
    return var_dict


def grid_subset(var_dict, lat_upper_bound, lat_lower_bound,
    long_upper_bound, long_lower_bound):
    """
    ...
    lat_indices = np.where((var_dict['lat'] <= lat_upper_bound) & (var_dict
        ['lat'] > lat_lower_bound))[0]
    long_indices = np.where((var_dict['long'] <= long_upper_bound) & (
        var_dict['long'] >= long_lower_bound))[0]
...
def interpolate_grid(var_dict, num_rows, num_cols):
    ...
    for key in var_dict.keys():
        if key in ['lat', 'long', 'time']:
            continue
        interp = RegularGridInterpolator((var_dict['time'][:], var_dict[
            'lat'], var_dict['long'][:]), var_dict[key][:, :, :])
        new_lat = np.linspace(var_dict['lat'][-1], var_dict['lat'][0], num_cols
            )
        new_long = np.linspace(var_dict['long'][0], var_dict['long'][-1],
            num_rows)
        X, Y, Z = np.meshgrid(var_dict['time'], new_lat, new_long, indexing
            ='ij')
...
def generate_met_dem_diagnostic_plots(old_ERA5_grid, ERA5_grid, ilats,
    ilons, iheights):
    """
    ...
    cont = ax1.contourf(lons, lats, temperature[0], cmap=cmap, transform=
        projection, levels=levels, vmin=vmin, vmax=vmax)
    ...
    cont = ax2.contourf(ilons, ilats, ERA5_grid['temperature'][0], cmap=
        cmap, transform=projection, levels=levels, vmin=vmin, vmax=vmax)
    ...
    cont = ax3.contourf(ilons, ilats, iheights, cmap=cmap, levels=20,
...
def get_met_bounds_from_DEM(model_setup, ERA5_grid, lat_array, lon_array,
    diagnostic_plots=False):
    from monarchs.DEM.load_DEM import export_DEM
    from monarchs.core.utils import find_nearest
    bounds = ['bbox_top_right', 'bbox_bottom_left', 'bbox_top_left',
        'bbox_bottom_right']
    bdict = {}
    for bound in bounds:
    ...
    iheights, ilats, ilons, dx, dy = export_DEM(model_setup.DEM_path,
        top_right=bdict['bbox_top_right'], bottom_left=bdict[
        'bbox_bottom_left'], top_left=bdict['bbox_top_left'], bottom_right=
        bdict['bbox_bottom_right'], num_points=model_setup.row_amount,
        diagnostic_plots=False, all_outputs=False, input_crs=model_setup.
        input_crs)
    ...
    for var in ERA5_grid.keys():
        if var in ['time']:
            ...
            continue
        new_ERA5_grid[var] = np.zeros((len(ERA5_grid['time']), model_setup.
            row_amount, model_setup.col_amount))
        for i in range(model_setup.row_amount):
            for j in range(model_setup.col_amount):
                lat_indices[i, j] = find_nearest(ERA5_grid['lat'],
                    lat_array[i, j])
                lon_indices[i, j] = find_nearest(ERA5_grid['long'],
                    lon_array[i, j])
                new_ERA5_grid[var][:, i, j] = ERA5_grid[var][:, int(
                    lat_indices[i, j]), int(lon_indices[i, j])]
    ERA5_grid = new_ERA5_grid
    if diagnostic_plots:
        generate_met_dem_diagnostic_plots(old_ERA5_grid, ERA5_grid, ilats,
            ilons, iheights)
    return ERA5_grid
...
if __name__ == '__main__':
    ...
    row_amount = 5
    col_amount = 5
    ERA5_vars_subset_interpolated = interpolate_grid(ERA5_vars, row_amount,
        col_amount)
    import cartopy.crs as ccrs
    fig, ax = plt.subplots()
    ...
    ax1.gridlines(draw_labels=True)
    frames = 364 * 24
    cont = ax1.contourf(lons, lats, T, cmap=cmap, transform=projection,
    ...
    cont = ax2.contourf(orig_lons, orig_lats, orig_T, cmap=cmap, transform=
        projection, levels=20, vmin=vmin, vmax=vmax)
