...
def hybrid_solver(cell, dz, dt, LW_in, SW_in, T_air, p_air, T_dp, wind, N_top=10, max_iter=50, tol=1e-6):
    """
    ...
    rho_cp = cell['rho'] * cell['Sfrac'] * cp_ice + (1.0 - cell['Sfrac'] -cell['Lfrac']) * cp_air + cell['Lfrac'] * cp_water
    ...
    for it in range(max_iter):
        ...
        for i in range(N_top):
            if i == 0:
                ...
                Q = sfc_flux(cell["melt"], cell["exposed_water"], cell["lid"], cell["lake"],
                             cell["lake_depth"], LW_in, SW_in, T_air, p_air, T_dp, wind, T_top[0])
                residual[i] = rho_cp[i]/dt * (T_top[i] - T_old[i]) - (Q - 5.670374e-8 * T_top[i]**4) - flux_down
                jac_diag[i] = rho_cp[i]/dt + 4*5.670374e-8*T_top[i]**3 + k_top[i]/dz
            elif i < N_top - 1:
                # Interior top layers
                flux_up = k_top[i-1]*(T_top[i] - T_top[i-1])/dz
                flux_down = k_top[i]*(T_top[i] - T_top[i+1])/dz
                residual[i] = rho_cp[i]/dt*(T_top[i]-T_old[i]) - ( -flux_up - flux_down)
                jac_diag[i] = rho_cp[i]/dt + k_top[i-1]/dz + k_top[i]/dz
            else:
                # Bottom of top-N region
                flux_up = k_top[i-1]*(T_top[i] - T_top[i-1])/dz
                flux_down = kappa_bottom[0]*(T_bottom_old[0] - T_top[i])/dz  # link to bottom
                residual[i] = rho_cp[i]/dt*(T_top[i]-T_old[i]) - (-flux_up - flux_down)
                jac_diag[i] = rho_cp[i]/dt + k_top[i-1]/dz + kappa_bottom[0]/dz
